...
def load_logs(experiment_directory, type):
    ...
    if type == "loss":
        ...
        for _name, mags in logs["mlp_param_magnitude"].items():
            ax.plot(mags)
        ax.set(xlabel="Epoch", ylabel="Magnitude", title="MLP Parameter Magnitude")
        ax.legend(logs["mlp_param_magnitude"].keys())
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Plot Model training logs")
